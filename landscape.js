...
function tree(u,v,r,h=3){let p=xy(u,v);branch(p,p.clone().add(new THREE.Vector3(.08,h*.7,0)),.11);for(let i=0;i<12;i++){const a=i*2.399,rr=r*(.3+.6*rand());const end=p.clone().add(new THREE.Vector3(Math.cos(a)*rr,h*(.6+.35*rand()),Math.sin(a)*rr));branch(p.clone().add(new THREE.Vector3(0,h*.5,0)),end,.022);for(let j=0;j<13;j++)foliage(end.clone().add(new THREE.Vector3((rand()-.5)*r*.8,(rand()-.5)*r*.6,(rand()-.5)*r*.8)),r*(.14+.12*rand()),i+j)}}
[[120,145,1.35,3.8],[265,160,1.2,3.5],[357,173,.75,2.2],[130,386,1.7,4.1],[810,936,3.5,6.2]].forEach(x=>tree(...x));
...
function palm(u,v,h){const p=xy(u,v),top=p.clone().add(new THREE.Vector3(.18,h,0));branch(p,top,.15);for(let k=0;k<16;k++){let a=k*2.399,len=2.3+(k%3)*.2;const points=[];for(let i=0;i<=12;i++){let t=i/12;points.push(top.clone().add(new THREE.Vector3(Math.cos(a)*len*t,.7*Math.sin(t*Math.PI)-.9*t*t,Math.sin(a)*len*t)))}for(let i=1;i<points.length;i++)branch(points[i-1],points[i],.014);const verts=[];for(let i=1;i<12;i++){let t=i/12;for(const side of [-1,1]){const b=points[i],w=.55*Math.sin(Math.PI*t),tip=b.clone().add(new THREE.Vector3(Math.cos(a+Math.PI/2)*w*side,-.25,Math.sin(a+Math.PI/2)*w*side));tip.add(new THREE.Vector3(Math.cos(a)*.25,0,Math.sin(a)*.25));verts.push(...b.toArray(),...tip.toArray(),...b.clone().add(new THREE.Vector3(Math.cos(a)*.11,0,Math.sin(a)*.11)).toArray())}}const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.Float32BufferAttribute(verts,3));g.computeVertexNormals();let m=new THREE.Mesh(g,materials[k%5]);m.name='Palma fronda pennata';m.castShadow=true;root.add(m)}}palm(690,748,6.3);palm(1238,392,4.8);
...
for(const [u,v]of [[458,732],[469,681]]){let p=xy(u,v,.8);let pot=new THREE.Mesh(new THREE.CylinderGeometry(.29,.2,.48,16),potmat);pot.position.copy(p).add(new THREE.Vector3(0,.24,0));pot.name='Vaso bianco';root.add(pot);for(let i=0;i<30;i++)foliage(p.clone().add(new THREE.Vector3((rand()-.5)*.6,.75+rand()*.6,(rand()-.5)*.6)),.13,i)}
const c=xy(464,705,.81);for(let i=0;i<65;i++){const a=rand()*Math.PI;let p=c.clone().add(new THREE.Vector3(Math.cos(a)*1.25,1.65+Math.sin(a)*.7,(rand()-.5)*.28));foliage(p,.1+rand()*.05,i);if(i%3===0){const m=new THREE.Mesh(new THREE.OctahedronGeometry(.065),petalmat);m.position.copy(p).add(new THREE.Vector3(.07,.03,.04));m.name='Bougainvillea fiore';root.add(m)}}
...
for(const [u,v] of [[122,270],[164,520]]){const p=xy(u,v);for(let k=0;k<12;k++){const a=k*2.399,rr=.9+rand()*.5;const top=p.clone().add(new THREE.Vector3(Math.cos(a)*rr,3.5+rand(),Math.sin(a)*rr));branch(p,top,.03);for(let j=0;j<10;j++)foliage(top.clone().add(new THREE.Vector3((rand()-.5)*.25,-j*.19,(rand()-.5)*.25)),.17,k+j)}}

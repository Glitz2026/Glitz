...
hedge([560,137],[982,174],1,.75);hedge([1030,355],[1030,655],.95,.9);hedge([768,682.6],[1004,696],.75,.75);hedge([210,653],[253,895],1.1,.6);// Tall hedge retains the original rear alignment and reaches the LED wall top.
const hedgeStart=root.children.length;const ha=xy(190,345),hb=xy(190,624),hn=Math.ceil(ha.distanceTo(hb)/.34);for(let i=0;i<=hn;i++){const p=ha.clone().lerp(hb,i/hn);for(let y=.28;y<=4.39;y+=.35)for(const side of [-1,1])foliage(p.clone().add(new THREE.Vector3(side*.18,y,0)),.27,(i+Math.floor(y*10))%5);}const screenHedge=new THREE.Group();screenHedge.name='Siepe alta dietro Back the Stage';for(const child of root.children.slice(hedgeStart))screenHedge.add(child);screenHedge.userData={height:4.65,alignment:[[190,345],[190,624]]};root.add(screenHedge);

...
for(const [u,v]of [[462,767],[518,767]]){let p=xy(u,v,.8);let pot=new THREE.Mesh(new THREE.CylinderGeometry(.29,.2,.48,16),potmat);pot.position.copy(p).add(new THREE.Vector3(0,.24,0));pot.name='Vaso bianco';root.add(pot);for(let i=0;i<30;i++)foliage(p.clone().add(new THREE.Vector3((rand()-.5)*.6,.75+rand()*.6,(rand()-.5)*.6)),.13,i)}
tree(548,774,.55,1.9,.8);
const c=xy(490,767,.81);for(let i=0;i<65;i++){const a=rand()*Math.PI;let p=c.clone().add(new THREE.Vector3((rand()-.5)*.28,1.65+Math.sin(a)*.7,Math.cos(a)*1.25));foliage(p,.1+rand()*.05,i);if(i%3===0){const m=new THREE.Mesh(new THREE.OctahedronGeometry(.065),petalmat);m.position.copy(p).add(new THREE.Vector3(.07,.03,.04));m.name='Bougainvillea fiore';root.add(m)}}

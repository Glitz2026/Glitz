...
hedge([560,137],[982,174],1,.75);hedge([1030,355],[1030,655],.95,.9);hedge([670,677],[1004,696],.75,.75);hedge([210,653],[253,895],1.1,.6);// Tall hedge retains the original rear alignment and reaches the LED wall top.
const hedgeStart=root.children.length;const ha=xy(190,237),hb=xy(190,624),hn=Math.ceil(ha.distanceTo(hb)/.34);for(let i=0;i<=hn;i++){const p=ha.clone().lerp(hb,i/hn);for(let y=.28;y<=4.39;y+=.35)for(const side of [-1,1])foliage(p.clone().add(new THREE.Vector3(side*.18,y,0)),.27,(i+Math.floor(y*10))%5);}const screenHedge=new THREE.Group();screenHedge.name='Siepe alta dietro Back the Stage';for(const child of root.children.slice(hedgeStart))screenHedge.add(child);screenHedge.userData={height:4.65,alignment:[[190,237],[190,624]]};root.add(screenHedge);
...
for(const [u,v]of [[480,703],[480,774]]){let p=xy(u,v,.8);let pot=new THREE.Mesh(new THREE.CylinderGeometry(.29,.2,.48,16),potmat);pot.position.copy(p).add(new THREE.Vector3(0,.24,0));pot.name='Vaso bianco';root.add(pot);for(let i=0;i<30;i++)foliage(p.clone().add(new THREE.Vector3((rand()-.5)*.6,.75+rand()*.6,(rand()-.5)*.6)),.13,i)}
// Removed circled small plant, screenshot 5.
const c=xy(480,738.5,.81);for(let i=0;i<65;i++){const a=rand()*Math.PI;let p=c.clone().add(new THREE.Vector3(Math.cos(a)*1.58004458,1.65+Math.sin(a)*.7,(rand()-.5)*.28));foliage(p,.1+rand()*.05,i);if(i%3===0){const m=new THREE.Mesh(new THREE.OctahedronGeometry(.065),petalmat);m.position.copy(p).add(new THREE.Vector3(.07,.03,.04));m.name='Bougainvillea fiore';root.add(m)}}
...
// Return of the black boundary at the lawn corner.
for(const [a,b] of [[[90,90],[78,92]],[[78,92],[78,262]]]){const aa=xy(...a),bb=xy(...b),n=Math.ceil(aa.distanceTo(bb)/.4);for(let j=0;j<=n;j++){const p=aa.clone().lerp(bb,j/n);fenceRod(p,p.clone().add(new THREE.Vector3(0,1.12,0)),.012);}fenceRod(aa.clone().add(new THREE.Vector3(0,1.12,0)),bb.clone().add(new THREE.Vector3(0,1.12,0)),.018);}
...
// Rear tree at [164,520] removed at owner request.

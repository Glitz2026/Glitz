...
- **[Feb 2026]** **Integrazione Piantina 3D** da repo GitHub `Glitz2026/Glitz` branch `integrazione-prenotazioni-3d`. Importati 4 file: `frontend/public/glitz-interattivo.html` (1.5MB, viewer standalone con Three.js), `frontend/src/components/Floorplan3D.jsx` (React wrapper iframe con GlitzMap bridge), `frontend/src/components/floorplanBridge.js` (helpers MODEL_ZONES + inventory), `docs/INTEGRAZIONE-3D.md`. Patch a `Floorplan.jsx` (toggle 3D↔2D di default 3D) e `EventDetail.jsx` (Prenota Tavolo scrolla alla piantina se `floorplan_enabled`). Verificato live su `/eventi/{id}` — vista isometrica renderizzata correttamente con zone cliccabili.

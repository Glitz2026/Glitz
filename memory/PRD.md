...
- **[Feb 2026]** **Piantina 3D pulita — pure render**. Rimosso completamente il rendering 2D dal `Floorplan.jsx` (SVG desktop + overlay fullscreen mobile, ~200 righe eliminate) e il toggle 3D/2D. Nel viewer `glitz-interattivo.html` nascosti via CSS `body.embedded`: `.table-marker`, `.zone-marker`, `#markers`, `#plan-toggle` ("Pianta"), `#top` ("2D" vista dall'alto). Toolbar ora mostra solo: Arco / + / − / ↺. Cache-bust `?v=16`.

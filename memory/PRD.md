...
- **[Feb 2026]** **Editabilità admin estesa**. (1) Zone piantina SEAVIEW e PRATO_BACK ora editabili da Admin > Piantina/Prezzi (`price_from`, `bottles`, `label`, `color`, `description`). (2) Nuovi settings `events_ticket_label` (testo bottone) + `events_ticket_url` (URL fallback globale se evento non ha `ticket_url` specifico). EventDetail.jsx usa entrambi con fallback. Admin form Contenuti > Titoli sezioni: blocco "Bottone Biglietti (globale)". (3) Home: sezione Club rinominata **"AREE DIVERSE, UNA SOLA NOTTE"** (da "Due Ambienti").
- **[Feb 2026]** **Sync major #2 da GitHub `Glitz2026/Glitz` main `7242c10`**. Importati: (1) Modello 3D **v15-v20** (villa/pool/portico/garden, Riva sofas rifinite, Back the Stage seating, PB lounges perimetro, arco lettering, Glitz Bar posizionamento, siepi ridotte, doppio subwoofer). (2) **Header ridisegnato**: nuovo logo GLITZ CLUB serif, layout pulito con cart/lang/login/hamburger. (3) **AdminLayout.jsx** — chrome dell'admin panel ridisegnato. (4) **CMS Contact info** — `ContactContext.jsx` + editor admin per email/telefono/indirizzo/social. (5) **SafeImage.jsx** — fallback graceful per immagini rotte (usato in Home + EventDetail su tutti gli `<img>` critici). (6) Endpoint `/api/admin/products/reorder` per riordino drag-and-drop prodotti. Cache-bust `?v=24`. **Preservato**: piantina 3D injection v23 (style + script custom), zone cards accordion 5 su una riga, gallery strips, "AREE DIVERSE, UNA SOLA NOTTE", 2 ambienti + 1 prodotto in home, ticket_label globale, dot di stato sopra tavolo selezionato. Importati: (1) **Shop Cart multi-item** con `CartContext.jsx` + `CartDrawer.jsx` (localStorage persistence, prodotti con size/quantity, checkout unico contact-info). (2) **Sistema Ordini**: nuovo endpoint `/api/orders` + `/api/admin/orders` + tab "Ordini" in admin. (3) **i18n IT/EN** con `LanguageContext.jsx` + `lib/i18n.js` (nav, footer, floorplan, booking, shop), toggle lingua in header. (4) **Password Login** con `/accedi` (`Login.jsx`), email/password oltre a Google, stesso session-cookie. (5) **Rate limiting** per-IP su endpoint pubblici (bookings/orders/private-events/newsletter). (6) **Fix table booking bug**: rifiuto/cancellazione ora auto-libera il tavolo, admin endpoint `/admin/events/{id}/tables/{num}/release` + bottone "libera tavolo". (7) **Fix race admin edit**: `update_event` non lascia più al client sovrascrivere `reserved_tables`. (8) **Bottle menu editor** per tutte e 5 le zone (marca + prezzo, aggiungi/rimuovi). (9) `httpx` + `stripe` aggiunti a requirements.txt. Preservato: piantina 3D v22, gallery strip, home reductions, "AREE DIVERSE".

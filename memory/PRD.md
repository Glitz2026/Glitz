...
- **[17 Feb 2026]** Floorplan v4: sostituiti i rettangoli bounding box con `<path>` SVG che ripercorrono le pareti reali di ciascun privé. `ZONE_POLYGONS` estratto tramite scansione pixel PIL della PNG. STAGE usa 2 sotto-tracciati (`M...Z M...Z`) per il cluster B0-B7 alto + "BACK THE STAGE" (B8-B15) basso. RIVA con parete diagonale in alto-sinistra + smusso in basso-destra. BAR L-shaped. Animazione pulsante `<animate>` su stroke-width e opacity applicata a `path` invece che `rect`.

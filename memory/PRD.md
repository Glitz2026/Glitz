...
- **[Feb 2026]** **Piantina 3D — Interazione ultra-premium v22**. (1) Vista iniziale su arco ridotta (solo arch-view.click, senza zoom extra) per mostrare più contesto. (2) **Hint animato "Pizzica o scorri per esplorare"** al centro all'apertura, fade out a 5s o al primo pointerdown/wheel/touchstart. (3) Layer rosso emissive sui mesh 3D. Zona selezionata → tutti in rosso medio. **Selezione specifica tavolo → SOLO quel tavolo rosso saturo, tutti gli altri clearTablePaint** (deselezione automatica). (4) Number label = pill rossa animata con **dot di stato** accanto al numero (🟢 available, 🔴 soldout, ⚫ unavailable, 🟡 unconfigured). (5) Bottone action: "Prenota tavolo B3". Cache-bust `?v=22`.

...
- **[Feb 2026]** **Piantina 3D — Interazione ultra-premium**. (1) Vista iniziale automatica sull'arco con doppio zoom (arch-view.click + 3x zoom-in). (2) Rimossi i marker HTML rossi a pallini: ora **layer rosso emissive** sui mesh 3D dei divani/poltrone (paintTable via `window.__glitz.model`). Zona selezionata → tutti i tavoli in rosso medio (emissiveIntensity 1.0 + color tint #ff4438). Tavolo selezionato → rosso saturo pieno (intensity 2.2 + color tint #ff2010). Rerender forzato via `controls.dispatchEvent({type:'change'})`. (3) Number label flottante sopra il tavolo selezionato (pill rossa animata). (4) Bottone action mostra **"Prenota tavolo B0"** invece del generico "Prenota". Wrapper su `GlitzMap.selectZone/selectTable/reset` mantiene stato ↔ UI. Cache-bust `?v=21`.

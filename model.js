...
import {furniture,continuousBench} from './furniture.js';
...
const root=new THREE.Group();root.name='GLITZ_CLUB';root.userData={version:'5.0',archDimensions:ARCH_SPEC,layoutSource:'Pianta schematica monocromatica della venue.png',tableIdsFromUserPlan:true,scaleBasis:'A3 PDF, nominal 1:200, hand digitized',surveyVerified:false,up:'Y',units:'metres'};
...
polygon('Percorso accesso',[[490,974],[570,993],[650,644],[1007,645],[1007,350],[1040,350],[1040,662],[705,684],[632,940],[1250,1075],[1240,1110]],.02,.07,C.path);
...
for(const [poly,y] of [[north,.6],[south,.9]]){polygon('BACK THE STAGE',poly,y,y+.04,C.white);polygon('BACK THE STAGE finitura',poly,y+.012,.024,C.stone)}
zones.push({id:'back',name:'BACK THE STAGE',coords:[...north,...south],y:.9,parts:[{coords:north,y:.6},{coords:south,y:.9}],anchor:xy(280,574,1.7),description:'Sedute morbide e tavolini tondi intorno al palco. Tavoli B0–B15.'});
zone('glitzbar','GLITZ BAR',yard,.8,[388,786],'Panche in muratura, tavoli in legno e sedie pieghevoli. Tavoli G1–G8.');
rail([[544,257],[943,257]],1.12);rail([[980,286],[980,352]],1.12);rail([[980,380],...terrace.slice(3,-1),[793,458]],1.12);rail([[708,466],[572,478],[544,257]],1.12);
rail([[202,234],[388,234]],.62);rail([[202,234],[202,322]],.62);rail([[202,365],[202,425]],.62);rail([[202,427],[202,653],[330,653]],.92);rail([[411,653],[512,653],[512,619]],.92);
...
stairs('Scala terrazza laterale',752,464,.6,1.1,3.1,1.1,Math.PI/2,3);stairs('Scala terrazza fondo',958,255,0,1.1,1.6,1.45,Math.PI/2,6);stairs('Scala pista',539,561,.1,.6,3.8,1.15,-.68,3);stairs('Scala privé giardino',370,660,.6,.9,1.9,.6,Math.PI/2,2);stairs('Scala esterna privé mare',204,343,0,.6,1.6,.9,Math.PI/2,3);stairs('Scala garden privé',215,884,0,.8,2.1,1.15,Math.PI/2,4);
const arch=createArch(xy(361,457));root.add(arch.root);
// Structural feet sit at the user-confirmed support level (1 m above the dancefloor).
for(const [x,floor] of [[-4.475,.9],[4.475,.6]]){const p=arch.root.position.clone();p.x+=x;p.y=(ARCH_SPEC.supportY+floor)/2;box('Appoggio arco quota confermata',p,.47,ARCH_SPEC.supportY-floor,.47,C.white)}
const screen=at('LED wall',231,457,2.6,5,2,.12,C.dark);screen.userData.sign='GLITZ';
...
for(const item of layout){
 const group=furniture(item.furniture,item.id);group.name='TABLE_'+item.id;group.userData={tableId:item.id,zoneId:item.zoneId,idConfirmedBySource:true,furnitureDimensionsEstimated:true};group.position.copy(xy(...item.planPosition,item.height));
 if(item.id==='G6')group.rotation.y=Math.PI/2;else if(['G7','G8'].includes(item.id))group.rotation.y=Math.PI;
 ...
 tables.push({...item,label:item.id,position:group.position.clone().add(new THREE.Vector3(0,.85,0)),capacity:null,minSpend:null,status:'unconfigured',source:'ID and layout from user schematic; furniture appearance from supplied photos',group});
}
...
polygon('Prato giardino centrale',[[650,658],[1006,658],[1043,934],[1094,968],[925,1040],[575,931]],.025,.06,'#496043');
...
ramp('Rampa ingresso verso piazzale',[[506,909],[577,929],[650,655],[572,655]],[.03,.03,.60,.60]);
ramp('Raccordo accesso Glitz Bar',[[529,808],[601,827],[609,798],[538,779]],[.8,.24,.30,.8]);
// Vine arch over garden lounge entry.
const archCenter=xy(464,705,.81);for(let i=0;i<24;i++){let a=i*Math.PI/24,b=(i+1)*Math.PI/24;rod('Pergola garden',archCenter.clone().add(new THREE.Vector3(Math.cos(a)*1.25,1.65+Math.sin(a)*.7,0)),archCenter.clone().add(new THREE.Vector3(Math.cos(b)*1.25,1.65+Math.sin(b)*.7,0)),.022,'#646550')}for(const x of [-1.25,1.25])rod('Montante pergola',archCenter.clone().add(new THREE.Vector3(x,0,0)),archCenter.clone().add(new THREE.Vector3(x,1.65,0)),.025,'#646550');
landscape(root,xy);
...
service('WC accessibile',1095,230,1.7,1.9);service('WC donne',1005,980,4.2,6.5);service('WC uomini',1413,1080,1.8,7.4);service('Deposito',194,759,2.1,3.8);
const pois=[['Palco DJ',314,457,2.1],['Pista',461,404,.12],['Main Bar',936,613,1.9],['Bar',467,180,1.4],['Gin xp',1165,490,1.6],['Ingresso',530,958,.3],['WC',1005,980,2.2]];
const hotspots=[];

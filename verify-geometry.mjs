...
const booth=m.root.getObjectByName('Console DJ parallela arco');assert.equal(booth.rotation.y,m.arch.root.rotation.y);
const localOffset=booth.position.clone().sub(m.arch.root.position).applyAxisAngle(new THREE.Vector3(0,1,0),-booth.rotation.y);assert.ok(Math.abs(localOffset.z+.42+.60)<1e-8);assert.ok(Math.abs(localOffset.x)<1e-8);
assert.equal(nodes.filter(o=>o.userData.model==='CDJ-3000X').length,4);assert.equal(nodes.filter(o=>o.userData.model==='DJM-A9').length,1);
assert.equal(nodes.filter(o=>o.name==='Pouf morbido senza schienale').length,16);
for(const o of nodes.filter(o=>o.name==='Pouf morbido senza schienale'))assert.ok(!o.getObjectByName('Poltrona sacco schienale'));

...
assert.equal(m.tables.length,50);assert.equal(new Set(m.tables.map(t=>t.id)).size,50);
...
assert.equal(m.tables.filter(t=>t.zoneId==='seaview').length,6);
assert.equal(m.tables.filter(t=>t.zoneId==='prato-back').length,4);

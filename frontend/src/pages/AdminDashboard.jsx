...
import { LogOut, Plus, Trash2, Edit, Upload, Calendar as CalIcon, FileText, HelpCircle, Image as ImgIcon } from "lucide-react";
...
export default function AdminDashboard() {
    ...
    const load = async () => {
        try {
            const [e, p, f, m] = await Promise.all([
                api.get("/events", { params: { published_only: false } }),
                api.get("/posts", { params: { published_only: false } }),
                api.get("/faqs"),
                api.get("/media"),
            ]);
            setEvents(e.data); setPosts(p.data); setFaqs(f.data); setMedia(m.data);
        } catch (err) {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
            ...
            <Tabs value={tab} onValueChange={setTab}>
                <TabsList className="bg-surface border border-white/10 grid grid-cols-2 lg:grid-cols-4 h-auto p-1">

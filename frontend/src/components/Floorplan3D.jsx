...
export default function Floorplan3D({ eventId, tables, reservedTables, getTableInfo, activeZone, ready, onSelect, onFallback }) {
    ...
    const firstZoneRun = useRef(true);
    ...
    useEffect(() => {
        if (!map) return;
        if (MODEL_ZONES[activeZone]) {
            map.selectZone(MODEL_ZONES[activeZone]);
            firstZoneRun.current = false;
        } else if (!firstZoneRun.current) {
            map.reset();
        }
    }, [map, activeZone]);
    ...
    return <div className="mb-6">
        ...
        {failed ? <div role="status" className="glass-card rounded-xl p-6">
            ...
            <iframe ref={frame} src="/glitz-interattivo.html?embedded=1&v=24" title="Mappa 3D interattiva del Glitz Club"
